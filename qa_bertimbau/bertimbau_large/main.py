...
from transformers import BertForQuestionAnswering, BertTokenizerFast, pipeline, TrainingArguments, Trainer, default_data_collator
import evaluate
from datasets import load_dataset
from pathlib import Path
from preprocessing import flatten_data, prepare_train_features
...
def run():
    # Definir los archivos de entrenamiento y validación
    train_file = './data/flat_squad-train-v1.1.json' \
        if Path('./data/flat_squad-train-v1.1.json').exists() else flatten_data('./data/squad-train-v1.1.json')
    ...
    # Cargar la métrica con la librería `evaluate`
    metric = evaluate.load('squad')
    ...
    batch_size = 16
    train_epochs = 2
    experiment_name = f'{train_epochs}_epochs_{model_type}_qa-2e-4'

    # Configuración de los argumentos de entrenamiento
    training_args = TrainingArguments(
        output_dir=f'./results/{experiment_name}',  # directorio de salida
